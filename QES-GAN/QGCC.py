...
class PQWGAN_CC():
    def __init__(self, image_size, channels, n_generators, n_qubits, n_ancillas, n_layers,
                 patch_shape):
        self.image_shape = (channels, image_size, image_size)
        self.critic = self.ClassicalCritic(self.image_shape)
        self.generator = self.QuantumGenerator(n_generators, n_qubits, n_ancillas, n_layers,
                                               self.image_shape, patch_shape)
    ...
    class QuantumGenerator(nn.Module):
        def __init__(self, n_generators, n_qubits, n_ancillas, n_layers, image_shape, patch_shape):
            ...
            self.params = nn.ParameterList(
                [nn.Parameter(torch.rand(n_layers, n_qubits, 3), requires_grad=True) for _ in
                 range(n_generators)])
        ...
        def forward(self, x):
            ...
            for sub_generator_param in self.params:
                ...
                for item in x:
                    sub_generator_out = self.partial_trace_and_postprocess(item,
                                                                           sub_generator_param).float().unsqueeze(
                        0)
            ...
            if special_shape:
                ...
                for i, img in enumerate(output_images):
                    for patches_done, j in enumerate(range(0, img.shape[0], pixels_per_patch)):
                        ...
                        starting_h = ((patches_done * self.patch_shape[1]) // self.image_shape[2]) * \
                                     self.patch_shape[0]
...
if __name__ == "__main__":
    gen = PQWGAN_CC(image_size=16, channels=1, n_generators=16, n_qubits=5, n_ancillas=1,
